...
def atomic_form_factor(atom, QList, inputCoeff=None):
    ...
    if atom not in aff_DF['element'].values:
        print(f'The atom \'{atom}\' is not available.\nThe available atoms are:\n', '\t'.join(
            map(str, aff_DF['element'].values)))
    else:
        if inputCoeff is None:
            coeff_values = aff_DF[aff_DF['element']
                                  == atom].iloc[:, 2:-2].values[0]
        else:
            coeff_values = inputCoeff
        sumData = 0.0
        for i in range(0, len(coeff_values) - 1, 2):
            sumData += coeff_values[i] * \
                np.exp(-coeff_values[i+1] * (QList/(4*np.pi))**2)
...
def neutron_scattering_lengths(rawTable=False):
    ...
    stream = resource_stream(
        __name__, 'Data/NeutronScatteringLengths_Corrected.csv')
...
class weight_RDF_for_scattering:
    def __init__(
        ...
        comps = re.sub(r"([A-Z])", r" \1",
                       composition.replace(' ', '')).split()
        compArr = []
        for atoms in comps:
        ...
        self.partialRDF = RDF_DataFrame.rename(
            columns={RDF_DataFrame.keys()[0]: 'r'})
            ...
            for atoms in atomArr:
                if atoms in self.isotopeDict.keys():
                    ...
                    for isotope in self.isotopeDict[atoms].keys():
                        bValue += self.isotopeDict[atoms][isotope] * neutronScatteringLengths.loc[neutronScatteringLengths['Isotope'].str.fullmatch(
                            isotope)]['Coh b'].values[0].real
                    bArr.append(bValue)
                else:
                    bArr.append(neutronScatteringLengths.loc[neutronScatteringLengths['Isotope'].str.fullmatch(
                        atoms)]['Coh b'].values[0].real)
        else:
            bArr = [neutronScatteringLengths.loc[neutronScatteringLengths['Isotope'].str.fullmatch(
                atomArr[i])]['Coh b'].values[0].real for i in range(len(atomArr))]

        QArr, SofQ = fourierbesseltransform(
            RDF_DataFrame.iloc[:, 0], RDF_DataFrame.iloc[:, 1]-1, unpack=True)
        self.QArrInterp = np.linspace(
            QArr[0], QArr[-1], len(QArr)*interpAmount)

        if self.ionsDict is not None:
            affArr = [atomic_form_factor(
                atomArr[i]+self.ionsDict[atomArr[i]], self.QArrInterp) for i in range(len(atomArr))]
        else:
            affArr = [atomic_form_factor(atomArr[i], self.QArrInterp)
        ...
        for column in RDF_DataFrame.keys()[1:]:
            ...
            SofQ_interp = interp1d(
                Q, SofQ, kind=interpType, bounds_error=False, fill_value=np.nan)(self.QArrInterp)
        ...
        totalgofr = 0
        for column in RDF_DataFrame.keys()[1:]:
            self.gofrNeutron[column] = self.weightArrayNeutron[column] * \
                RDF_DataFrame[column] / self.weightTotalNeutron
        ...
        totalSofQ = 0
        for column in RDF_DataFrame.keys()[1:]:
            self.SofQNeutron[column] = self.weightArrayNeutron[column] * \
                self.unweightedSofQ[column] / self.weightTotalNeutron
        ...
        totalSofQ = 0
        for column in RDF_DataFrame.keys()[1:]:
            self.SofQXray[column] = self.weightArrayXray[column] * \
                self.unweightedSofQ[column] / self.weightTotalXray
        ...
        for column in RDF_DataFrame.keys()[1:]:
            r, gofr = fourierbesseltransform(self.SofQXray['Q'].iloc[:cutAt100InvAng:interpAmount].to_numpy(
            ), self.SofQXray[column].iloc[:cutAt100InvAng:interpAmount].to_numpy(), unpack=True)
            self.gofrXray['r'] = r
    ...
    def plot_SofQNeutron(self, axes=None, **kwargs):
        ax = plt.axes(axes)
        ax.plot(self.SofQNeutron['Q'], self.SofQNeutron['Total'],
                'k-', lw=2, label='Total', **kwargs)
        for column in self.SofQNeutron.keys()[1:-1]:
            ax.plot(self.SofQNeutron['Q'], self.SofQNeutron[column],
                    '--', lw=1, label=column, **kwargs)
        return ax

    def plot_gofrNeutron(self, axes=None, **kwargs):
        ax = plt.axes(axes)
        ax.plot(self.gofrNeutron['r'], self.gofrNeutron['Total'],
                'k-', lw=2, label='Total', **kwargs)
        for column in self.gofrNeutron.keys()[1:-1]:
            ax.plot(self.gofrNeutron['r'], self.gofrNeutron[column],
                    '--', lw=1, label=column, **kwargs)
        return ax

    def plot_SofQXray(self, axes=None, **kwargs):
        ax = plt.axes(axes)
        ax.plot(self.SofQXray['Q'], self.SofQXray['Total'],
                'k-', lw=2, label='Total', **kwargs)
        for column in self.SofQXray.keys()[1:-1]:
            ax.plot(self.SofQXray['Q'], self.SofQXray[column],
                    '--', lw=1, label=column, **kwargs)
        return ax

    def plot_gofrXray(self, axes=None, **kwargs):
        ax = plt.axes(axes)
        ax.plot(self.gofrXray['r'], self.gofrXray['Total'],
                'k-', lw=2, label='Total', **kwargs)
        for column in self.gofrXray.keys()[1:-1]:
            ax.plot(self.gofrXray['r'], self.gofrXray[column],
                    '--', lw=1, label=column, **kwargs)
        return ax
    ...
    def plot_scatteringLengths(self, axes=None, xray=True, neutron=True, **kwargs):
        ...
        for atoms in self.compositionTable.index:
            if neutron:
                ...
                ax.plot(self.QArrInterp, np.full(len(self.QArrInterp),
                        self.compositionTable.b[atoms]), '-', lw=1, label=labelValue+' b', **kwargs)
            if xray:
                labelValue = atoms
                if self.ionsDict is not None:
                ...
                ax.plot(
                    self.QArrInterp, self.compositionTable.aff[atoms], '-', lw=1, label=labelValue+' aff', **kwargs)
    ...
    def calc_num_density(self, density):
        ...
        num_density : float
        """
        num_density = constants.Avogadro/10**24 * density / \
